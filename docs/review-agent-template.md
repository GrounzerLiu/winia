...
## 工作方式
- **严格只读**：不运行 cargo build / cargo test 等任何编译或验证命令，不修改任何文件，
  不 git add/commit。只通过阅读代码、文档和 git diff 审查。
...
3. 验证说明：按约定严格只读，未运行构建/测试；如需编译验证，由主 agent 另行执行
...
5. 进度控制：子 agent 若预计耗时较长，应先返回中间进度；主 agent 等待超时
   建议 ≤3 分钟，超时先询问进度，仍无产出再中断并接手审查。
