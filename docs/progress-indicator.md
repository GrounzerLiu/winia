...
| 额外旋转 | 90° 步进 keyframes（300ms 到 90°，保持至 1500ms，随后每 300ms 动画+保持至 4800ms 到 360°） | **全 Linear**——Compose `using` 属于段起点帧，`90f at 300 using Decelerate` 只作用于 hold 段 [300,1500]（值不变，零视觉作用）；0ms 为隐式补入帧（LinearEasing），故所有动画段实际全线性 |
| 进度呼吸 | 0.1→0.87→0.1 keyframes（3000ms 峰值） | 上升段 Linear（0ms 隐式帧）；下降段 Standard (0.2, 0, 0, 1)（`0.87 at 3000 using Standard`） |
