...
use winia::ui::component::{ButtonExt, RadioExt, RadioGroupExt, TextExt};
use winia::ui::item::Size;
use winia::ui::layout::{AlignItems, ColumnExt, RowExt, StackExt};
use winia::shared::ForEachOption;
pub fn radio_test(w: &WindowContext) -> Item {
    ...
    w.column(children!(
        ...
        .padding_start(16)*/
        w.column(
            ["A", "B", "C", "D", "E", "F", "S"].iter().for_each_option(|value| {
                let str = *value;
                w.row(children!(
                    w.radio(StyledText::from(str), &selected_value, None).item(),
                    w.text(str).item().margin_start(16)
                ))
                .align_items(AlignItems::Center)
                .item()
                .padding_start(16)
