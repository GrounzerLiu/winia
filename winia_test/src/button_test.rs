...
use winia::ui::component::{ButtonExt, ButtonType, RectangleExt, TextExt};
...
pub fn button_test(w: &WindowContext) -> Item {
    ...
    let selected = Shared::from(false);
    w.column(
        // w.text(&text)
        //     .editable(false)
        //     .item()
        w.button(&text).item().on_click({
            let mut click_count = 0;
            move |_| {
                click_count += 1;
                text.set(format!("Button clicked {} times", click_count));
            }
        }) + w.rectangle(Color::TRANSPARENT).item().size(Size::Fill, 16)
            + w.button("Selectable button")
                .selectable(true)
                .selected(&selected)
                .item()
                .on_click({
