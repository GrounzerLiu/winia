...
pub fn ranked_suggestions<I: Iterator<Item = (Argb, u32)>>(
    ...
    for hct in color_hct {
        ...
        if options.filter
            && (hct.get_chroma() < K_CUTOFF_CHROMA || proportion < K_CUTOFF_EXCITED_PROPORTION)
        {
            continue;
        }
    ...
    for difference_degrees in (15..=90).rev() {
        ...
        for (hct, _) in &scored_hct {
            if !chosen_colors.iter().any(|chosen_hct| {
                diff_degrees(hct.get_hue(), chosen_hct.get_hue()) < difference_degrees as f64
            }) {
