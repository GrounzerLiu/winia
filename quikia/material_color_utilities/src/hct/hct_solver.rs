...
use crate::utils::{Argb, argb_from_linrgb, int_from_lstar, K_PI, matrix_multiply, sanitize_degrees_double, signum, Vec3, vec3, y_from_lstar};


const K_SCALED_DISCOUNT_FROM_LINRGB: [[f64; 3]; 3] = [
    [
...
const K_LINRGB_FROM_SCALED_DISCOUNT: [[f64; 3]; 3] = [
    [
        1373.2198709594231,
        -1100.4251190754821,
        -7.278681089101213
...
const K_Y_FROM_LINRGB: [f64; 3] = [0.2126, 0.7152, 0.0722];

const K_CRITICAL_PLANES: [f64; 255] = [
 ...
 */
fn sanitize_radians(angle: f64) -> f64 { (angle + K_PI * 8.0) % (K_PI * 2.0) }
...
fn hue_of(linrgb: Vec3) -> f64 {
    let scaled_discount = matrix_multiply(linrgb, K_SCALED_DISCOUNT_FROM_LINRGB);
...
fn lerp_point(source: Vec3, t: f64, target: Vec3) -> Vec3 {
    Vec3 {
        a: source.a + t * (target.a - source.a),
        b: source.b + t * (target.b - source.b),
        c: source.c + t * (target.c - source.c),
    }
}

fn get_axis(vector: Vec3, axis: i32) -> f64 {
    match axis {
        0 => vector.a,
        1 => vector.b,
        2 => vector.c,
 ...
 */
fn set_coordinate(source: Vec3, coordinate: f64, target: Vec3, axis: i32) -> Vec3 {
    let t = intercept(get_axis(source, axis), coordinate, get_axis(target, axis));
    lerp_point(source, t, target)
}

fn is_bounded(x: f64) -> bool { 0.0 <= x && x <= 100.0 }
 ...
 */
fn nth_vertex(y: f64, n: i32) -> Vec3 {
    let k_r = K_Y_FROM_LINRGB[0];
    let k_g = K_Y_FROM_LINRGB[1];
    let k_b = K_Y_FROM_LINRGB[2];
...
fn critical_plane_below(x: f64) -> i32 { (x - 0.5).floor() as i32 }

fn critical_plane_above(x: f64) -> i32 { (x - 0.5).ceil() as i32 }
        ...
        if get_axis(left, axis) != get_axis(right, axis) {
            ...
            for _i in 0..8 {
                if (r_plane - l_plane).abs() <= 1 {
                    ...
                    let m_plane = ((l_plane + r_plane) as f64 / 2.0).floor() as usize;
                    let mid_plane_coordinate = K_CRITICAL_PLANES[m_plane];
...
fn inverse_chromatic_adaptation(adapted: f64) -> f64 {
    ...
    let base = 0.0_f64.max(27.13 * adapted_abs / (400.0 - adapted_abs));
...
// ===========================================================
    let viewing_conditions = DEFAULT_VIEWING_CONDITIONS;
    let t_inner_coeff = 1.0 / (1.64 - 0.29_f64.powf(viewing_conditions.background_y_to_white_point_y)).powf(0.73);
...
// ===========================================================
        ...
        let alpha = if
        chroma == 0.0 || j == 0.0 { 0.0 } else { chroma / j_normalized.sqrt() };
        ...
        let scaled = vec3(r_c_scaled, g_c_scaled, b_c_scaled);
        let linrgb = matrix_multiply(scaled, K_LINRGB_FROM_SCALED_DISCOUNT);
// ===========================================================
// Operations inlined from Cam16 to avoid repeated calculation
// ===========================================================
        ...
        let k_g = K_Y_FROM_LINRGB[1];
        let k_b = K_Y_FROM_LINRGB[2];
        let fnj = k_r * linrgb.a + k_g * linrgb.b + k_b * linrgb.c;
        if fnj <= 0.0 {
            return 0;
        }
...
pub fn solve_to_int(hue_degrees: f64, chroma: f64, lstar: f64) -> Argb {
    ...
    let hue_radians = hue_degrees / 180.0 * K_PI;
...
fn solve_to_cam(hue_degrees: f64, chroma: f64, lstar: f64) -> Cam {
