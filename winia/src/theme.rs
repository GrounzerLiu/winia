...
#[derive(Clone)]
pub struct StateStyles<T: Clone> {
    pub enabled: T,
    pub disabled: T,
    pub hovered: T,
    pub focused: T,
...
impl<T: Clone> StateStyles<T> {
    pub fn enabled(enabled: T) -> Self {
        Self {
            enabled: enabled.clone(),
            disabled: enabled.clone(),
            hovered: enabled.clone(),
            focused: enabled.clone(),
            pressed: enabled,
        }
    }
    ...
    pub fn get(&self, state: ItemState) -> &T {
        match state {
            ...
            ItemState::Hovered => &self.hovered,
            ItemState::Focused => &self.focused,
            ItemState::Pressed => &self.pressed,
        }
    }
}
