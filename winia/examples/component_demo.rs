...
fn component_demo(ctx: &mut ComposeCtx) {
    ...
    // The layout direction comes from the shared chrome's settings sheet (it provides
    // LocalLayoutDirection for the whole window), so every Row/Text/padding start-end still mirrors
    // with it — this demo's own inline direction toggle is gone, and with it the conflict of two
    // switches writing the same CompositionLocal.
Column::new()
    ...
    winia::run_app!(|ctx| {
        ...
        WiniaTheme::auto(ctx, |ctx| {
            Window::new()
                ...
                .build(ctx, |ctx| {
                    settings::shell("Component Polish Demo", ctx, component_demo);
                });
        });
    });
}