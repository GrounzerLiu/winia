...
// The direction is the chrome's to give now (its settings sheet): this demo's own toggle
// provided it for the whole page, and two switches wrote the same CompositionLocal.
Column::new()
        ...
        .build(ctx, |ctx| {
            ...
            Row::new()
                ...
                .build(ctx, |ctx| {
                    // The title is the chrome's now; this row keeps the Primary/Secondary case.
...
fn main() {
    winia::run_app!(|ctx| {
        Window::new()
            ...
            .title("TabRow Demo")
            .build(ctx, |ctx| {
                settings::shell("TabRow Demo", ctx, tab_row_demo);
            });
    });
}